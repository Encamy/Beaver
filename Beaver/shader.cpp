...
GLuint LoadShaders(const char * vertex_file_path, const char * fragment_file_path) {
	...
	if (VertexShaderStream.is_open()) {
		...
		log.LOG_ERROR("Impossible to open "+ std::string(vertex_file_path) +". Are you in the right directory ?");
		return 0;
	}
	...
	if (InfoLogLength > 0) {
		...
		log.LOG_INFO(std::string(&VertexShaderErrorMessage[0]));
	}
	...
	if (InfoLogLength > 0) {
		...
		log.LOG_INFO(std::string(&FragmentShaderErrorMessage[0]));
	}



	// Link the program
	log.LOG_TRACE("Linking program");
